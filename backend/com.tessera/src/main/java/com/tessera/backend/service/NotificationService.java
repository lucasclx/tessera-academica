// src/main/java/com/tessera/backend/service/NotificationService.java
...
public class NotificationService {
    ...
    private void sendRealTimeNotification(User user, Notification notification) {
        try {
            ...
            messagingTemplate.convertAndSendToUser(
                user.getEmail(), // Usando email como identificador único
                "/topic/notifications",
                dto
            );
            
            // Também enviar summary atualizado
            NotificationSummaryDTO summary = getNotificationSummary(user);
            messagingTemplate.convertAndSendToUser(
                user.getEmail(),
                "/topic/notification-summary",
                summary
            );
        } catch (Exception e) {
    ...
    public void markAsRead(Long notificationId, User user) {
        ...
        if (!notification.isRead()) {
            ...
            // Enviar summary atualizado via WebSocket
            NotificationSummaryDTO summary = getNotificationSummary(user);
            messagingTemplate.convertAndSendToUser(
                user.getEmail(),
                "/topic/notification-summary",
