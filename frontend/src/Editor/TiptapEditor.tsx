...
// Ícones substitutos para Subscrito e Sobrescrito
const CustomSubscriptIcon = () => <span style={{ fontFamily: 'monospace', fontWeight: 'bold' }}>x₂</span>;
const CustomSuperscriptIcon = () => <span style={{ fontFamily: 'monospace', fontWeight: 'bold' }}>x²</span>;
    ...
    return (
      <div className="editor-toolbar sticky top-0 z-10 bg-gray-50 border-b border-gray-200 p-1 sm:p-2 flex flex-wrap items-center gap-x-1 gap-y-0.5 min-h-[46px]">
        ...
        <div className="flex items-center border-r border-gray-200 pr-1 mr-1">
          ...
          <ToolbarButton onClick={() => editor.chain().focus().toggleSubscript().run()} isActive={editor.isActive('subscript')} title="Subscrito"><CustomSubscriptIcon /></ToolbarButton>
          <ToolbarButton onClick={() => editor.chain().focus().toggleSuperscript().run()} isActive={editor.isActive('superscript')} title="Sobrescrito"><CustomSuperscriptIcon /></ToolbarButton>
        </div>
        <div className="flex items-center border-r border-gray-200 pr-1 mr-1">
    ...
    <div className={className}>
      ...
      {editable && editor && !editor.isDestroyed && isEditorReallyReady && (
        ...
        >
          ...
          <button onClick={() => editor.chain().focus().toggleSubscript().run()} disabled={!editable || !editor.can().chain().focus().toggleSubscript().run()} className={`p-1.5 rounded ${editor.isActive('subscript') ? 'bg-primary-500 text-white' : 'hover:bg-gray-700'} disabled:opacity-50`} title="Subscrito"><CustomSubscriptIcon /></button>
          <button onClick={() => editor.chain().focus().toggleSuperscript().run()} disabled={!editable || !editor.can().chain().focus().toggleSuperscript().run()} className={`p-1.5 rounded ${editor.isActive('superscript') ? 'bg-primary-500 text-white' : 'hover:bg-gray-700'} disabled:opacity-50`} title="Sobrescrito"><CustomSuperscriptIcon /></button>
          {onAddComment && (
            <button
              onClick={() => {
