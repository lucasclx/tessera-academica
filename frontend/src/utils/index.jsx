...
import { 
  ...
  Popover, Tooltip, Switch, FormControlLabel, FormGroup, FormHelperText
} from '@mui/material';
import { 
...
export const StatusChip = ({ status, type = 'document', variant = 'filled', size = 'small', showIcon = true, sx }) => {
  ...
  if (showIcon && IconComponent) {
    if (typeof IconComponent === 'function') {
      iconElement = <IconComponent fontSize="small" />;
      ...
      <Table stickyHeader={stickyHeader} size={size}>
        <TableHead>
          <TableRow sx={{ '& th': { bgcolor: 'grey.100' }}}>
            ...
            {onActionMenuClick && <TableCell align="right" sx={{ width: 50, pr:1 }}>Ações</TableCell>}
          </TableRow>
        </TableHead>
        <TableBody>
          {data.length === 0 ? (
              ...
              >
                {columns.map((column) => (
                  <TableCell key={column.id} align={column.align || 'left'}>
                    {column.render ? column.render(row) : (row[column.id] === undefined || row[column.id] === null ? '-' : String(row[column.id]))}
                  </TableCell>
                ))}
                {onActionMenuClick && (
        ...
        openConfirmation(
            ...
            async (itemToDelete) => {
                try {
                    ...
                    if (typeof deleteFn !== 'function') { toast.error('Função de exclusão não implementada.'); return; }
                    await deleteFn(itemToDelete.id);
                    toast.success(`"${itemToDelete.title || itemToDelete.name || itemToDelete.id}" excluído.`);
                    reload(); 
                } catch (deleteError) {
                    console.error('Erro ao excluir item:', deleteError);
                    toast.error(deleteError.response?.data?.message || 'Falha ao excluir.');
                }
            },
    ...
    return (
      <Container maxWidth="lg" sx={{pb:4}}>
        <PageHeader
          ...
          onBackClick={pageSubtitle ? () => actualNavigate(-1) : undefined }
          backButton={!!pageSubtitle}
        />
            ...
            <Grid container spacing={2} alignItems="center">
              {searchFilterConfig && (
                <Grid item xs={12} md={mainFilterConfig || otherFilters.length > 0 ? 6 : 12}>
                  <TextField fullWidth variant="outlined" placeholder={searchFilterConfig.placeholder || "Buscar..."} value={search} onChange={(e) => setSearch(e.target.value)} InputProps={{ startAdornment: ( <InputAdornment position="start"> <Search /> </InputAdornment> ), }}/>
                </Grid>
              )}
              {mainFilterConfig && (
                <Grid item xs={12} md={searchFilterConfig || otherFilters.length > 0 ? (otherFilters.length > 0 ? 3 : 6) : 12}>
                  <TextField select fullWidth variant="outlined" label={mainFilterConfig.label} value={filter} onChange={(e) => setFilter(e.target.value)}>
                    {mainFilterConfig.options?.map(option => ( <MenuItem key={option.value} value={option.value}> {option.label} </MenuItem> ))}
                  </TextField>
                </Grid>
              )}
              {otherFilters.map((f, index) => (
                 <Grid item xs={12} sm={6} md={3} key={f.id || `other-filter-${index}`}>
                   <TextField select={f.type === 'select'} fullWidth variant="outlined" label={f.label} value={f.value || ''} onChange={f.onChange} InputLabelProps={f.type === 'date' ? { shrink: true } : {}} type={f.type === 'date' ? 'date' : undefined}>
                     {f.type === 'select' && f.options?.map(option => ( <MenuItem key={option.value} value={option.value}> {option.label} </MenuItem> ))}
                   </TextField>
                 </Grid>
              ))}
            </Grid>
        ...
        <DataTable
          ...
          onRowClick={(item) => {
             ...
             if (viewAction?.onClick) { viewAction.onClick(item); }
             else if(createPath && typeof createPath !== 'string' && createPath.viewPath && item.id) { actualNavigate(createPath.viewPath.replace(':id', item.id)); }
        ...
        {selectedItemForAction && availableRowActions.length > 0 && (
          <MenuComponent anchorEl={actionMenuAnchorEl} open={Boolean(actionMenuAnchorEl)} onClose={handleActionMenuClose} PaperProps={{ elevation: 2, sx: { minWidth: 180 } }}>
            {availableRowActions.map((action, index) => {
              ...
              const isDisabled = typeof action.disabled === 'function' && action.disabled(selectedItemForAction, currentUser);
              ...
              return (
                <MenuItem key={action.label + index} onClick={() => { if(typeof action.onClick === 'function') action.onClick(selectedItemForAction); handleActionMenuClose(); }} disabled={isDisabled} sx={{color: action.isDestructive ? 'error.main' : 'inherit'}}>
        ...
        <ConfirmDialog open={confirmDialogOpen} onClose={() => setConfirmDialogOpen(false)} onConfirm={confirmDialogProps.onConfirm} title={confirmDialogProps.title} message={confirmDialogProps.message} confirmText={confirmDialogProps.confirmText || "Confirmar"} variant={confirmDialogProps.variant || 'danger'} loading={loading}/>
      </Container>
    );
  };
}; //
