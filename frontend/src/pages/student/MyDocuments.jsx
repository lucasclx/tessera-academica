...
const MyDocuments = () => {
  ...
  // MODIFICADO: Renderização principal com skeleton loading
  if (error) {
    return (
      <Container>
  ...
  return (
    <Container maxWidth="lg">
      ...
      <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 2 }}>
        ...
        <Button variant="contained" color="primary" startIcon={<Add />} size="large" onClick={handleCreateNew}>
          Nova Monografia
        </Button>
      </Box>

      {/* MODIFICADO: Skeleton loading para filtros */}
      {loading ? (
        <Box sx={{ mb: 3 }}>
          <Card sx={{ p: 2 }}>
      ...
      {loading ? (
        // SKELETON LOADING - Mostra estrutura da tabela
        <TableSkeleton 
          rows={6}
          columns={['45%', '15%', '20%', '15%', '5%']}
        />
      ) : documents.length === 0 ? (
      ...
      {/* FAB para mobile */}
      <Fab color="primary" aria-label="add" sx={{position: 'fixed', bottom: 24, right: 24}} onClick={handleCreateNew}>
        <Add />
      </Fab>

      {/* Menu de ações */}
      <Menu anchorEl={menuAnchor} open={Boolean(menuAnchor)} onClose={handleMenuClose}>
        <MenuItem onClick={() => { if (selectedDocument) handleViewDocument(selectedDocument.id); handleMenuClose(); }}>
          <ListItemIcon><Visibility fontSize="small" /></ListItemIcon>
          <ListItemText>Visualizar</ListItemText>
        </MenuItem>
        <MenuItem onClick={() => { if (selectedDocument) handleEditDocument(selectedDocument.id); handleMenuClose(); }}>
          <ListItemIcon><Edit fontSize="small" /></ListItemIcon>
          <ListItemText>Editar</ListItemText>
        </MenuItem>
        {selectedDocument?.status === 'DRAFT' && (
          <MenuItem onClick={() => { setDocumentToDelete(selectedDocument); setDeleteDialog(true); handleMenuClose(); }}>
      ...
      <Dialog open={deleteDialog} onClose={() => setDeleteDialog(false)}>
        ...
        <DialogContent>
          <Typography>Tem certeza que deseja excluir "{documentToDelete?.title}"? Esta ação não pode ser desfeita.</Typography>
        </DialogContent>
        <DialogActions>
